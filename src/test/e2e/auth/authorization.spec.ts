...
/**
 * E2E tests for authorization and access control.
...
test.describe("Authorization", () => {
  ...
  test("should prevent user from accessing another user's project", async ({ page }) => {
    ...
    // Try to access user1's project via API
  ...
  test("should prevent user from updating another user's project", async ({ page }) => {
    ...
    // Try to update user1's project via API
    const { status, body } = await authenticatedApiRequest(
      page.request,
      page,
      `/api/projects/${project.id}`,
  ...
  test("should prevent user from deleting another user's project", async ({ page }) => {
    ...
    // Try to delete user1's project via API
    const { status, body } = await authenticatedApiRequest(
      page.request,
      page,
      `/api/projects/${project.id}`,
